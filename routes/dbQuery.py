...
# 注意：get_projects_by_client 的完整版本在第 578 行（包含 has_reviewed 欄位）
...
# 注意：get_bids_by_contractor 的完整版本在第 572 行（包含 has_reviewed 和 client_id 欄位）
...
## routes/dbQuery.py
# dbQuery.py
async def db_upload_file_db(conn, bid_id: int, original_filename: str, stored_filename: str, file_path: str, uploader_id: int):
    """
    上傳檔案並實現版本控管
    - 查詢已有的版本數
    - 設定新版本號
    - 將舊版本設為 inactive
    - 同時更新 submissions 表（向後兼容）

    Args:
        bid_id: 投標 ID
        original_filename: 原始檔案名稱（用戶上傳時的檔名）
        stored_filename: 儲存的檔案名稱（UUID 檔名）
        file_path: 檔案路徑
        uploader_id: 上傳者 ID
    """
    try:
        async with conn.cursor(row_factory=rows.dict_row) as cur:
            ...
            await cur.execute(
                ...
                FROM project_files
                WHERE bid_id = %s AND file_type = 'deliverable'
                """,
                (bid_id,)
            )
            version_result = await cur.fetchone()
            new_version = version_result["max_version"] + 1

            # 2. 將該 bid 的所有舊版本設為 inactive
            await cur.execute(
                """
            ...
            # 3. 插入新版本（is_active = true）
            await cur.execute(
                """
                INSERT INTO project_files
                (project_id, uploader_id, bid_id, file_type, original_filename,
                 stored_filename, file_path, version_number, is_active)
                VALUES (%s, %s, %s, 'deliverable', %s, %s, %s, %s, true)
                """,
                (project_id, uploader_id, bid_id, original_filename, stored_filename, file_path, new_version)
            )

            print(f"[DB DEBUG] Uploaded deliverable v{new_version} for bid {bid_id}: {original_filename} (stored as {stored_filename})")

            # ========= 向後兼容：更新 submissions 表 =========
            # 檢查是否已有上傳記錄
            await cur.execute(
            ...
            if existing:
                ...
                await cur.execute(
                    ...
                    WHERE id = %s
                    """,
                    (stored_filename, file_path, uploader_id, existing["id"])
                )
                print(f"[DB DEBUG] Updated submission for bid {bid_id}: {stored_filename}")
            else:
                # 新增
                await cur.execute(
                    """
                    ...
                    """,
                    (bid_id, project_id, stored_filename, file_path, uploader_id)
                )
                print(f"[DB DEBUG] Inserted new submission for bid {bid_id}: {stored_filename}")
...
# === 獲取版本歷史 ===
async def db_get_file_versions(conn, bid_id: int):
    """
    獲取指定 bid 的所有交付版本（按版本號降序）
    """
    async with conn.cursor(row_factory=rows.dict_row) as cur:
        await cur.execute(
            """
